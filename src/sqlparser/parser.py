...
MAX_TABLES = 4
MAX_CONDITIONS = 6  # Disjunction (cela grupa u zagradi) broji se kao 1
...
class _Parser:
    ...
    def parse_query(self) -> Query:
        ...
        return Query(select=select, tables=tables, where=where, order_by=order_by)
    ...
    def parse_table_list(self) -> tuple:
        tables = [self.expect(TokenKind.IDENT, "table name").text]
        while self.peek().kind is TokenKind.COMMA:
            self.advance()
            tables.append(self.expect(TokenKind.IDENT, "table name").text)
    ...
    def parse_disjunction(self):
        ...
        members = [self._parse_or_member()]
        while self.at_keyword("OR"):
            self.advance()
            members.append(self._parse_or_member())
        self.expect(TokenKind.RPAREN, "')' or OR")
        if len(members) == 1:  # (a=1) → obican Condition, ne degenerisana Disjunction
            return members[0]
        return Disjunction(tuple(members))
    ...
    def _parse_or_member(self) -> Condition:
        # unutar zagrada gramatika dozvoljava samo: attr op literal
        left = self._parse_attr()
        op = self.expect(TokenKind.OP, "comparison operator").text
